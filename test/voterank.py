        ...
        if album_name != "Q":
            if album_name in albumDic.keys():
                for i in timeDic.values():
                    ...
                    total_album_point = getAlbumPoint(total_ranks, album_name)
                    num_of_data = len(filtered_list)
                    print("{} 입덕 : {}x{}명".format(i, int(total_album_point / num_of_data), num_of_data))
            else:
                print("앨범아이디를 찾을 수 없습니다")
        ...
        if album_name != "Q":
            if album_name in timeDic.values():
                ...
                for name in albumDic.keys():
                    total_album_point = getAlbumPoint(total_ranks, name)
                    num_of_data = len(filtered_list)
                    print(f"{album_name}입덕 {num_of_data}명 : {name} 앨범 점수 {int(total_album_point / num_of_data)}점")
            elif album_name == "전체":
                music_dic = createMusicCountDic(data_list)
                total_ranks = getTotalRanks(music_dic)
                for name in albumDic.keys():
                    total_album_point = getAlbumPoint(total_ranks, name)
                    print(f"전체 {len(data_list)}명 : {name} 앨범 점수 {int(total_album_point / len(data_list))}점")
            else:
